...
workbook.xlsx.readFile('./generate/dimensions/BW FITTINGS/TEE/tee.xlsx')
.then(wb => {
  ...
  for (let row = 2; row < rowCount +1 ; row++) {
    ...
    let myObject = {
      ...
      'dimensions': {
        'outsideDiameterRun': {
          'display': 'outside diameter at bevel',
          'imperial': {
        ...
        'centerToEndRun': {
          'display': 'center to end dimension at run size',
          'imperial': {
        ...
        'centerToEndOutlet': {
          'display': 'center to end dimension at outlet size',
          'imperial': {
  ...
  fs.writeFile('./generate/dimensions/BW FITTINGS/TEE/tee.json', JSON.stringify(myCollection), function(err) {
    if (err) {
      console.log(err);
    } else {
      console.log('success');
